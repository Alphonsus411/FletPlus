...
- **Versión objetivo**: la versión objetivo vigente es **Flet 0.85.3** y la baseline mínima de validación en CI es **Flet 0.80.x** (`>=0.80,<0.81`); el workflow instala explícitamente `flet==0.85.3` para el target `0.85`.
...
> Referencia operativa: consulta la sección ["Upgrade de Flet (paso a paso)"](https://github.com/FletPlus/FletPlus/blob/main/docs/tooling.md#upgrade-de-flet-paso-a-paso) dentro de la [Política de upgrade de Flet y release](https://github.com/FletPlus/FletPlus/blob/main/docs/tooling.md#politica-de-upgrade-de-flet-y-release).

> Contrato de versión vigente: ver [Contrato de versión vigente](https://github.com/FletPlus/FletPlus/blob/main/docs/tooling.md#contrato-de-version-vigente) para la definición única y sincronizada entre empaquetado, CI y validación local.
...
En los Pull Requests hacia `main` o `develop` se ejecuta únicamente
