...
Aplicación fullstack generada con la CLI de FletPlus. Esta plantilla separa la interfaz Flet, la lógica Python local, los modelos compartidos, la documentación del proyecto y los scripts de despliegue.
...
Usa el build por defecto para empaquetar la interfaz y conservar la estructura backend/shared dentro del proyecto:
...
- `src/backend/services.py`: servicios Python, API local o lógica de dominio consumible desde la app Flet.
- `src/shared/models.py` y `src/shared/config.py`: modelos, configuración y utilidades comunes entre frontend y backend.
- `docs/README.md`: punto de partida para documentación generada del proyecto.
- `deploy/README.md`: espacio reservado para scripts por plataforma y automatizaciones de release.
