...
**Versión objetivo de migración (estado objetivo en CI)**: `flet==0.85.3` (job `latest-migration-target`).
**Estado oficial del target**: ✅ **validado contra `flet==0.85.3`**, último patch disponible en PyPI para la serie `0.85.x` durante la revisión del 2026-07-07.
...
| CI target (`flet-version-matrix`) | `flet==0.85.3` (`latest-migration-target`) | Consolidar compatibilidad funcional sobre el patch bloqueado `0.85.3` | `.github/workflows/reusable-quality.yml` |
